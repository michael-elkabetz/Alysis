...
export const executionRepository = {
  ...
  async getCostStatsByVersion(analysisId: string): Promise<RawVersionCostStats[]> {
    const results = await db
      .select({
        versionId: schema.executionLogs.versionId,
        totalPromptTokens: sql<number>`coalesce(sum((${schema.executionLogs.tokenUsage}->>'prompt')::int), 0)::int`,
        totalCompletionTokens: sql<number>`coalesce(sum((${schema.executionLogs.tokenUsage}->>'completion')::int), 0)::int`,
      ...
      .where(
        and(
          ...
          isNotNull(schema.executionLogs.versionId)
        )
      )
      .groupBy(schema.executionLogs.versionId)

    return results as RawVersionCostStats[]
  },
}
